...
def apply_styles():
    css_file = os.path.join(css_dir, "style.json")
    if os.path.exists(css_file):
        with open(css_file, "r") as file:
            styles = json.load(file)
        if "console" in styles:
            console.config(bg=styles["console"].get("bg", "#FFFFFF"), fg=styles["console"].get("fg", "#000000"))
            console.tag_configure("url", foreground=styles["console"].get("url_fg", "blue"))
        if "entry" in styles:
            entry.config(bg=styles["entry"].get("bg", "#FFFFFF"), fg=styles["entry"].get("fg", "#000000"))
        if "button" in styles:
            for btn in (send_button, file_button, save_button, select_button):
                btn.config(bg=styles["button"].get("bg", "#0078d4"), fg=styles["button"].get("fg", "#FFFFFF"), font=styles["button"].get("font", "Arial 10 bold"))
        if "listbox" in styles:
            user_listbox.config(bg=styles["listbox"].get("bg", "#FFFFFF"), fg=styles["listbox"].get("fg", "#000000"))

def run_gui():
    global entry, console, file_path, user_listbox, guild_id, channel_id, send_button, file_button, save_button, select_button
    ...
    user_listbox = tk.Listbox(sidebar, width=40, height=20)
